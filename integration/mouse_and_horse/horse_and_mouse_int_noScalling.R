...
# Write the counts matrix to matrix.mtx
writeMM(counts_matrix, file = "matrix.mtx")

# Write gene names to genes.tsv
write.table(data.frame(gene_names), file = "genes.tsv", sep = "\t", 
            quote = FALSE, col.names = FALSE, row.names = FALSE)

# Write barcodes to barcodes.tsv
write.table(data.frame(barcodes), file = "barcodes.tsv", sep = "\t", 
            quote = FALSE, col.names = FALSE, row.names = FALSE)
...
# Optionally, re-normalize the data if needed
...
RenameGenesSeurat <- function(obj, newnames) {
  ...
  # Rename gene names in counts slot
  if (!is.null(RNA$counts)) {
    if (length(rownames(RNA$counts)) == length(newnames)) {
      rownames(RNA$counts) <- newnames
    } else {
      warning("Mismatch in counts slot: Skipping renaming for counts slot.")
    }
  }
  
  # Rename gene names in data slot
  if (!is.null(RNA$data)) {
    if (length(rownames(RNA$data)) == length(newnames)) {
      rownames(RNA$data) <- newnames
    } else {
      warning("Mismatch in data slot: Skipping renaming for data slot.")
    }
  }
  
  # Rename gene names in scale.data slot
  if (!is.null(RNA$scale.data)) {
    if (length(rownames(RNA$scale.data)) == length(newnames)) {
      rownames(RNA$scale.data) <- newnames
    } else {
      warning("Mismatch in scale.data slot: Skipping renaming for scale.data slot.")
    }
  }
...
# Apply the function
mouse <- RenameGenesSeurat(obj = mouse, newnames = final_gene_list)
