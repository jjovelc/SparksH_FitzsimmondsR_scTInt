...
RenameGenesSeurat <- function(obj, newnames) {
  ...
  RNA <- obj[["RNA"]]
  existing_names <- rownames(GetAssayData(RNA, slot = "counts"))
  
  print(paste("Existing gene count:", length(existing_names)))
  print(paste("New gene count:", length(newnames)))
  
  if (length(existing_names) != length(newnames)) {
    stop("Unequal gene sets: nrow(RNA) != length(newnames)")
  }
  
  # Ensure the new names are unique
  newnames <- make.unique(newnames)
  
  print("Renaming genes in all relevant slots.")
  
  for (slot in c("counts", "data")) {
    if (!is.null(GetAssayData(RNA, slot = slot))) {
      data <- GetAssayData(RNA, slot = slot)
      print(paste("Renaming slot:", slot))
      print(paste("Data dimensions:", dim(data)))
      rownames(data) <- newnames
      RNA <- SetAssayData(RNA, slot = slot, new.data = data)
    }
  }
...
# Check if the length of final_gene_list matches the number of rows in the RNA assay
if (length(final_gene_list) != nrow(mouse[["RNA"]])) {
  stop("The number of new gene names does not match the number of rows in the RNA assay.")
}
